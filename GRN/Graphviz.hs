...
graphvizWithNodeFormatter :: (Graph g, Show a, Show b) => (a->String)
...
graphviz = graphvizWithNodeFormatter sl

graphvizWithNodeFormatter fun g t (w, h) p@(pw', ph') o =
    "digraph "++t++" {\n"
	    ++"\tmargin = \"0\"\n"
	    ++o2s o
	    ++"\tratio = \"fill\"\n"
    ...
    where   n = labNodes g
            ...
            sn (node, a)    | fun a == "" = ""
                            | otherwise = '\t':(show node ++ fun a ++ "\n")
            se (n1, n2, b) = '\t':(show n1 ++ " -> " ++ show n2 ++ sl b ++ "\n")

clustgraphviz g t (w, h) p@(pw', ph') o =
    let n = labNodes g
	e = labEdges g
	ns = concatMap sn n
	es = concatMap se e
